...
<div class="wrapper" v-attributes="'wrapper'">
  <div class="listbox form-control" v-if="schema.listBox" :disabled="disabled">
    <div class="list-row" v-for="item in items" :class="{'is-checked': isItemChecked(item)}">
      <label>
        <input :id="getFieldID(schema, true)" type="checkbox" :checked="isItemChecked(item)" :disabled="disabled" @change="onChanged($event, item)" :name="getInputName(item)" v-attributes="'input'">{{ getItemName(item) }}
      </label>
    </div>
  </div>
